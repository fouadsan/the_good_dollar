...
function addDataToDom(data, size, visible) {
  ...
  productsContainer.classList.contains("not-visible")
  ...
  data.forEach((el) => {
    ...
    if (currentUrl == rootUrl + "shop/")
      visible <= size
        ...
        : (productCountEl.textContent = `Showing ${size} of ${size} results`);
    ...
    setTimeout(() => {
      ...
      if (el.is_cart) cardEl.classList.add("add__cart");

      if (el.is_fav) cardEl.classList.add("add__fav");
...
function typeView(viewBtn, oppositeEl, listViewClass) {
  ...
  viewBtn.addEventListener("click", () => {
    if (!viewBtn.classList.contains("active")) {
      ...
      viewBtn.firstElementChild.classList.contains("not-visible")
    ...
    if (cardEls)
      cardEls.forEach((cardEl) => {
        if (
          e.matches &&
          cardEl.className == "card col-lg-12 col-md-12 col-sm-6 col-12" &&
          cardEl.firstElementChild.classList.contains(listViewClassName)
        ) {
          cardEl.className = "card col-lg-4 col-md-12 col-sm-6 col-12";
    ...
    if (scrollTop + clientHeight >= scrollHeight) {
      ...
      if (visible <= size) {
        ...
        if (
          ...
          !productsContainer.lastElementChild.classList.contains("no__data")
        ) {
...
function addToWishOrCart(btnEl, class_name, id) {
  ...
  _productImg = _productImgEl.style.backgroundImage
    .slice(4, -1)
    .replace(/"/g, "");

  let _productName = document.getElementById(`product-name-${id}`).textContent;
  let _currentPrice = document
    .getElementById(`product-current-price-${id}`)
    .textContent.slice(1);
  ...
  if (cardinfoEl.classList.contains(class_name)) {
    ...
    $.ajax({
      ...
      success: function (response) {
        // delete
        if (response.total_items) totalItems = response.total_items;
        headerEls.forEach((headerEl) => {
          headerEl.textContent = totalItems;
        });
      },
...
if (
  currentUrl != shopUrl + "cart_screen" &&
  currentUrl != shopUrl + "wishlist_screen"
)
  getData();
...
if (currentUrl == shopUrl) startDOM();
